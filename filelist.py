        ...
        if os.path.isdir(os.path.abspath(base_folder)):
            if not base_folder.endswith(os.sep) and base_folder != '.' and args.list_only == False: # if the directory doesn't end with a slash don't add it
                ...
                for file in os.listdir(base_folder):
                    if os.path.isfile(file):
                        if base_folder.endswith(os.sep):
                            dest.append(base_folder + os.path.basename(file))
                        else:
...
def formatFilePrint(file_path):
    try :
        stat_info = os.stat(os.path.abspath(file_path))# Obtient les informations sur le fichier
    except:
        print("Error: %s is not a valid file or folder" % file_path)
        return None
